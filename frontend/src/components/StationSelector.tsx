...
export default function StationSelector({
  ...
  const select = (
    <div className="relative">
      <select
        ...
        className={`bg-aree-card border-aree-border text-aree-body focus:border-aree-accent w-full appearance-none rounded-lg border outline-none disabled:opacity-60 ${
          compact ? "py-1.5 pr-8 pl-2.5 text-[12px]" : "py-2.5 pr-10 pl-3 text-sm"
        }`}
      >
        <option value="">
      ...
      <ChevronDown
        className={`text-aree-muted pointer-events-none absolute top-1/2 -translate-y-1/2 ${
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      {state.data ? (
        <div className="text-aree-dim flex flex-wrap items-center gap-x-3 gap-y-1 text-[11px]">
          <span>
